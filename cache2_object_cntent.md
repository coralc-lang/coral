...
- `max_size_gb` — soft cap. When exceeded, `crl store gc` trims.
...
`crl build` handles the link/copy transparently.
...
### Project A changes a comment in `coral-lexer/lexer.crl`
...
Where does `~/.coral/projects` come from? Whenever `crl build` runs in a project, it appends the project path (idempotent). Over time this lists every project that has ever used the store. Stale entries get dropped when their path no longer exists.

Alternative for CI / containers: **no automatic GC**. Set `max_size_gb`, run `crl store gc --force` on a cron, accept that the store is a cache and losing it just means recompilation.
...
Disk corruption is real. `crl store verify`:
...
`crl store verify --delete-bad` removes corrupt objects. Their projects recompile on next build (cache miss, not fatal).
...
Advisory lock file with PID + start time. `crl store gc --break-lock` for stale locks.
...
```
crl store stat                    # size, object count, hit rate
crl store list                    # list all objects (hash, size, module)
crl store cat <hash>              # dump .meta for an object
crl store get <hash> <outfile>    # extract object to a file
crl store put <file>              # add an object manually (rare)
crl store verify [--delete-bad]   # rehash everything
crl store gc [--force]            # evict per policy
crl store path <hash>             # print canonical path
crl store prune                   # drop unreferenced objects (scan-based)
```
...
Or per-invocation: `crl build --no-store`.
...
Order of operations in `crl build` for a module:
...
| Verification | `crl store verify`, optional on-read |
...
- Write the **`object_hash` implementation** in `.crl`, including the strip-paths/strip-time normalization pass.
...
- Design **`crl store`** as a standalone binary vs a subcommand of `crl`.
